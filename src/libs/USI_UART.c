...
{  
    PORTB |= (1<<PB1) | (1<<PB0);         // Enable pull up on USI DO, DI and SCK pins. (And PB3 because of pin change interrupt)   
    DDRB  &= ~((1<<PB1) | (1<<PB0));        // Set USI DI, DO and SCK pins as inputs.  
    USICR = 0;                                            // Disable USI.
    TIFR  = 0;                                       // Clear Timer0 OVF interrupt flag.
    ...
    if( USI_UART_status.ongoing_Transmission_From_Buffer )      
    {
        ...
        else
        {
            ...
            else
            {
                ...
                DDRB  &= ~(1<<PB0);        // Set USI DI, DO and SCK pins as inputs.  
                PORTB |= (1<<PB0);         // Enable pull up on USI DO, DI and SCK pins. (And PB3 because of pin change interrupt)   
                USICR  =  0;                                            // Disable USI.
                GIFR   =  (1<<PCIF);                                    // Clear pin change interrupt flag.
                PCMSK |=  (1<<PCINT0);
                GIMSK |=  (1<<PCIE);                                   // Enable pin change interrupt for PB3:0.
...
ISR(TIMER0_COMPA_vect) {
    ...
    OCR0A = 104;                    // Shift every (103+1)*8 cycles
    USICR  = (0<<USISIE)|(1<<USIOIE)|                         // Enable USI Counter OVF interrupt.
             (0<<USIWM1)|(1<<USIWM0)|                         // Select Three Wire mode.
             (0<<USICS1)|(1<<USICS0)|(0<<USICLK)|             // Select Timer0 OVER as USI Clock source.
             (0<<USITC);                                           
